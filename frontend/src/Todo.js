		...
		const retgreen = (
			<div className='greencard' key={index}>
				...
				<div className='actions'>
					...
					<button className='btn-green' onClick={ () => props.editFunction(index) }>Edit</button>
				</div>
				{modalIsOpen && <Modal removeFunction={ () => props.removeFunction(index) } closeModal={desactivateModal}/>}
				{modalIsOpen && <Backdrop closeBackdrop={desactivateModal}/>}
	 		</div>
		)
		...
		const retred = (	
			<div className='redcard' key={index}>
				...
				<div className='actions'>
					...
					<button className='btn-red' onClick={ () => props.editFunction(index) }>Edit</button>
				</div>
				{modalIsOpen && <Modal removeFunction={ () => props.removeFunction(index)} closeModal={desactivateModal}/>}
				{modalIsOpen && <Backdrop closeBackdrop={desactivateModal}/>}
	 		</div>
